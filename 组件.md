...
| DoCheck             | ngDoCheck()                        | 组件、指令             | 每个变更检测周期中调用                                       |
| AfterContentInit    | ngAfterContentInit()               | 组件                   | 内容投影进组件后调用，在组件整个生命周期中，只会执行一次，没有投影，立即调用。 |
| AfterContentChecked | ngAfterContentChecked()            | 组件                   | 投影内容变更检测后触发。初始化过程，在AfterContentInit 之后触发 |
| AfterViewInit       | ngAfterViewInit()                  | 组件                   | **初始化组件视图及其子视图后触发**，是AfterContentInit 的补充 |
| AfterViewChecked    | ngAfterViewChecked()               | 组件                   | 组件视图及其子视图完成变更检测后触发。                       |
