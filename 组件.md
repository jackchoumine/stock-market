...
组件和指令拥有自己的生命周期，从创建、呈现、更新和销毁的特定时期，执行特定的函数。当组件呈现之后，就会为每个子组件启动生命周期，直到整个应用渲染。
...
`constructor` 不属于生命周期，但是会第一个执行，我们也可在构造函数中做一些工作。

`ngDestroy` 和其他带有 `init`的函数，在整个生命周期中，只会执行一次。
...
| ------------------- | ------------------------------- | ---------------------- | ------------------------------------------------------------ |
| OnChanges           | ngChanges(changes:simpleChange) | 组件、指令             | 指令被 set 后调用，每当指令的 input 属性被改变时调用         |
| OnInit              | ngOnInit()                      | 组件、指令             | 初始化时执行，只会执行一次。此处可调用服务器接口，从关注隔离和可测试的角度看，在该函数中请求接口，比构造函数更适合。 |
| DoCheck             | ngDoCheck()                     | 组件、指令             | 组件检查绑定或者变化的一种方法，在该函数中，可通知组件变化，这时可将变更检测策略改为 OnPush |
| AfterContentInit    | ngAfterContentInit()            | 组件                   | 在组件投影期间触发，在组件整个生命周期中，只会执行一次，没有投影，立即调用。 |
| AfterContentChecked | ngAfterContentChecked()         | 组件                   | 检查到输入变化时触发，初始化过程，在AfterContentInit 之后触发 |
| AfterViewInit       | ngAfterViewInit()               | 组件                   | AfterContentInit 的补充，当所有直接用在模板中的子组件初始化及它们的视图的绑定更新后触发。 |
| AfterViewChecked    | ngAfterViewChecked()            | 组件                   | 所有子视图检查并更新完毕之后触发。                           |
| OnDestroy           | ngOnDestroy()                   | 组件、指令             | 组件或指令即将被销毁并从UI中移除时调用。可进行一些清理工作，比如清除定时器 |
