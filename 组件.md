...
默认情况下，ng 会对 UI 中的每个绑定值检查，如果值一变化，就更新 UI。但是随着英语越来越大，默认的检查会有性能损失，而我们希望可以决定更新的时机。
...
组件可重用才是我们想要的，函数会更不同的参数得到不同的返回，组件类似，可根据不同的输入，显示不同的 UI。
...
| AfterContentChecked | ngAfterContentChecked()            | 组件              | 投影内容变更检测后触发。初始化过程，在 AfterContentInit 之后触发                                                                                                                                       |
